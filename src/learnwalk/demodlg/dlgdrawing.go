...
type myDrawingDlg struct {
	*walk.Dialog
	paintWidget *walk.CustomWidget
}

func RunDrawingDlg(owner walk.Form) (int, error) {
	mw := new(myDrawingDlg)
...
func (dlg *myDrawingDlg) drawStuff(canvas *walk.Canvas, updateBounds walk.Rectangle) error {
	...
	bounds := dlg.paintWidget.ClientBounds()
